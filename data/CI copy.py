...
class Closure_Invariants():
    ...
    def FTCI(self, imgs):
        if self.ehtim:
            for imgdata in imgs:
                self.template.xdim = imgdata.shape[-2]
                self.template.ydim = imgdata.shape[-1]
                imgdata = np.float_(np.ndarray.flatten(imgdata))
                self.template._imdict['I'] = imgdata
                self.template._imdict['Q'] = np.zeros_like(imgdata)
                self.template._imdict['U'] = np.zeros_like(imgdata)
                self.template._imdict['V'] = np.zeros_like(imgdata)

                

                obs = self.template.observe(self.ehtarray, self.tint_sec, self.tadv_sec, self.tstart_hr, self.tstop_hr, self.bw_hz, mjd = self.mjd, timetype='UTC', ttype='DFT', noise=False, verbose=False)
                ci = obs.ClosureInvariants()
